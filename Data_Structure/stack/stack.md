...
class Stack {
  ...
  push(value) {
    ...
    if (this.length === 0) {
      ...
      const holdingPointer = this.top; // old top
      this.top = newNode;
...
myStack.push("CC");
// Stack {
//   top: Node { value: 'CC', next: Node { value: 'BB', next: [Node] } },
//   bottom: Node { value: 'AA', next: null },
//   length: 3
// }
myStack.peek();
// Node {
//   value: 'CC',
//   next: Node { value: 'BB', next: Node { value: 'AA', next: null } }
// }
myStack.pop();
myStack.pop();
myStack.pop();
...
여기서 `ptrNode->next = head->next`를 실행하면 `head->next`가 스택의 가장 윗부분 노드를 가리키므로 새로운 노드가 추가되면 해당 노드의 _**next**_ 값에 `head->next`값에 저장된 주소를 넣는다. 그러면 새로 추가된 노드의 _**next**_ 값은 이전에 추가된 노드를 가리키게 된다. 그다음 `head->next`값을 현재 새로 추가한 노드로 만든다. 따라서 `head->next`값은 다시 가장 윗부분 노드를 가리킨다.
