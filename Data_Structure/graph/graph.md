...
#### 인접 행렬 기반 그래프 : 정방 행렬 이용

> 정방 행렬은 가로세로 길이가 같은 행렬이다. 2차원 배열로 표현한다.

- 무방향 그래프

  - 연결되었으면 **1**, 안결되어 있지 않으면 **0**
  - `[m][n]` = `[n][m]` 이며 대각선기준 대칭

![graph7](./images/graph7.svg)

<br>

- 방향 그래프
  - 예를들어 A에서 B방향이 연결되었으면 `[0][1]`을 1로 표시
  - 대칭을 이루지 않음

![graph8](./images/graph8.svg)

<br>

#### 인접 리스트 기반 그래프 : 연결리스트 이용

- 무방향 그래프

![graph9](./images/graph9.svg)

<br>

- 방향 그래프
  - 각 정점이 가리키는 정점만 연결리스트에 넣음

![graph10](./images/graph10.svg)
