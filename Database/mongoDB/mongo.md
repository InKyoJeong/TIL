...
- 몽고디비는 자바스크립트 문법을 사용하고, RDBMS가 아니라 SQL을 사용하지 않는 **NoSQL**임
  - NoSQL은 고정된 테이블이 없고 컬렉션이라는 개념이 있다. 컬럼을 따로 정의하지 않음
  - MySQL과 달리 JOIN 기능이 없음
- 몽고디비를 사용하는 이유는 확장성과 가용성 때문
  - 데이터의 일관성을 보장해주는 기능이 약한 대신 데이터를 빠르게 넣고, 쉽게 서버에 데이터 분산이 가능
- MySQL의 **테이블, 로우, 컬럼**을 몽고디비에서는 **컬렉션, 다큐먼트, 필드** 라고 함
